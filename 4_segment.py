...
def core(file):
    with tempfile.NamedTemporaryFile(delete=True) as tf:
        df = pd.read_csv(f"./data/clean.csv/{file}")
        df['text'] = df.text.str.replace("(\s|,)+"," ",regex=True)
        df.text.to_csv(tf,index=True,header=None)

        proc = subprocess.Popen(["./stanford-segmenter-2020-11-17/segment.sh","pku", str(tf.name), "UTF-8","0"],
        ...
        df['text'] = ss
        df.to_csv(f"./data/segmented.csv/{file}")
        return int(file.split(".")[0])

files = os.listdir("./data/clean.csv")
files.sort(key=lambda x: int(x.split(".")[0]))
filel = len(files)
os.makedirs("./data/segmented.csv",exist_ok=True)
